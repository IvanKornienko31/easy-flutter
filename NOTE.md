...
_После этого, возможно, потребуется перезапустить VS Code (или нажать `F1` -> `TypeScript: Restart TS Server`), и красные подчеркивания исчезнут._
...
2.  **Сниппет (`{@render ...}`):** Это "кусочек" разметки, который передается _внутрь_ компонента как данные.
